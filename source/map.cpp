...
class Map {
	...
	void initAsPrototype(int x, int y) {
		for (int i = 0; i < y; ++i) {
			...
			for (int j = 0; j < x; ++j) {
				if ((i != 0) and (i != y-1)) {	//if a middle row
					if ((j != 0) and (j != x-1)) {	//if middle column
						if (rand() % 20 == 1) {
							map[i].push_back(Tile(176, 0x67, false, false));
						} else {
							...
							map[i].push_back(Tile(toPushBack, col, true, true));
						}
					} else {
						map[i].push_back(Tile(205, 0x87, false, false));	//||
					}
				} else {
					map[i].push_back(Tile(205, 0x87, false, false));		//=
				}
	...
	vector<int> initAsBasicCave(int x, int y, int rooms) {
